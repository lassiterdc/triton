...
//  Local functions
//-----------------------------------------------------------------------------
static void   execRouting(double *exchange_q, double *swmm_newDepth, const double dt2D); //TRITON
...
//=============================================================================
int DLLEXPORT  swmm_run(const char *f1, const char *f2, const char *f3) //TRITON
{
	return 0;
}
...
//
{
...
#endif
    {
        ...
        project_init();
		  ...
		  Nobjects[SUBCATCH]=0; //TRITON
        //it is imposed to 0 to replace the SWMM's runoff computation by the TRITON model 
...
int DLLEXPORT swmm_step(double *elapsedTime, double *exchange_q, double *swmm_newDepth, const double dt2D) //TRITON
//
...
#endif
    {
        ...
        if ( NewRoutingTime < RoutingDuration )
        {
            ...
            //     (NewRoutingTime is updated)
				//
            execRouting(exchange_q, swmm_newDepth, dt2D); //TRITON
        }
...
//  Purpose: advances the simulation by a fixed number of seconds.
{
    ...
    *elapsedTime = 0.0;
    ...
    do
    {
        swmm_step(elapsedTime,trash1,trash2,0.0); //TRITON. Just to make it compile
    } while (*elapsedTime > 0.0 && !ErrorCode);
...
void execRouting(double *exchange_q, double *swmm_newDepth, const double dt2D) //TRITON 
//
//  Input:   none
//  Output:  none
...
//
{
...
#endif
    {
        ...
        if ( DoRouting )
            routing_execute(RouteModel, routingStep, exchange_q, swmm_newDepth); //TRITON
        else
...
//  Purpose: saves current results to binary output file.
{
    ...
    if (fabs(NewRoutingTime - ReportTime)<1e-8 || NewRoutingTime >= ReportTime) //TRITON (change due to machine accuracy problems)
    {
...
int DLLEXPORT swmm_report(const char *f1) //TRITON
//
//  Input:   none
//  Output:  returns an error code
